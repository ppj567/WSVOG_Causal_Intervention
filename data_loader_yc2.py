...
class YC2_train_data(Dataset.Dataset):
    ...
    def calc_class_balance(self):
        ...
        plt.bar(range(self.num_classes), sorted_class_obj)
        plt.savefig("1.jpg")
        return class_obj
...
class YC2_test_data(Dataset.Dataset):
    ...
    def get_sample(self, file_name):
        ...
        box = np.ones((len(gt_box), num_frames_1fps, 5)) * -1.0  # -1 if no object appears
        for i in gt_box:
        ...
        return seg_feature, seg_label, rpn_box, box#, seg_name
                    
        
    def __getitem__(self, index):
        file_name = self.file_name_list[index]
        seg_feature, seg_label, rpn_box, box= self.get_sample(file_name)#, seg_name 
        result = [seg_feature, seg_label, rpn_box, box]#spatio_box, frm_id , seg_name
        return result
