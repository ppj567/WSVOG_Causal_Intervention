...
class WSTOG(nn.Module):
    ...
    def ACL_Temporal(self, QR_frm_weight_p, embedding_p, input_pos_label, input_pos_feature):
        ...
        temporal_memorybank2_maxpool = F.normalize(torch.max(self.temporal_memorybank.memory.view(self.temporal_mem_size, -1, R_shape[-1]), dim=1)[0], dim=-1)
        ...
        Rn_updated = self.temporal_memorybank.memory[sim_idx].view(R_shape)
        ...
        for n in range(self.temporal_memorybank.memory.shape[0]):
            ...
            if num_all_high > 0:
                ...
                self.temporal_memorybank.update_weight(high_mean, n)
                cnt += 1
            else:
                high_mean = 0
    ...
    def ACL_Spatial(self, QR_p, embedding_p, input_pos_label, input_pos_feature):
        ...
        sim_idx = (torch.max(torch.mm(R, self.memorybank.memory.permute(1, 0)), dim=-1)[1])
        Rn_updated = self.memorybank.memory[sim_idx].view(R_shape)
        ...
        high_sim = (torch.max(torch.mm(R_high, self.memorybank.memory.permute(1, 0)), dim=-1)[1])
        ...
        for n in range(self.memorybank.memory.shape[0]):
            ...
            if num_all_high > 0:
                ...
                self.memorybank.update_weight(high_mean, n)
                cnt += 1
            else:
                high_mean = 0
